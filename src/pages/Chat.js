...
class Chat extends Component {
  ...
  handleFormChat = (event) => {
    ...
    formChatNew["id_yangdichat"] =
      this.state.dataIsiChat.id_yangdichat != parseInt(id)
        ? this.state.dataIsiChat.id_yangdichat
        : this.state.dataIsiChat.id_pengechat;
    this.setState(
      {
  ...
  handleBalasChat = (e) => {
    ...
    axios
      .post("http://localhost:8000/chat/balas", this.state.formChat)
      .then((res) => {
        ...
        <div className="body height-90  d-flex justify-content-between ">
          ...
          <div className="w-75 bg-success br-20 ml-4 body-chat">
            ...
            <div className="mx-4 body-chat-center">
                  ...
                  <>
                    {isi.id_pengechat == parseInt(this.state.id) ? (
                      <div className="container-chat">
                        <img
                          src="https://images.unsplash.com/flagged/photo-1557786458-77474e6ff1bb?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=880&q=80"
                          alt="Avatar"
                        />
                        <p>{isi.isi_chat}</p>
              ...
              {this.state.isiChatSaya.length > 0 ? (
                <form
                  className="form-inline input-chat"
                  onSubmit={this.handleBalasChat}
                >
                  <div className="form-group mb-2 w-75">
                    <input
                      ...
                      name="isi_chat"
                      onChange={this.handleFormChat}
                    />
                  </div>
                  <button type="submit" className="btn btn-success mb-2 w-25">
                    Kirim
                  </button>
                </form>
              ) : (
